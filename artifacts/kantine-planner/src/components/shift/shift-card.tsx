...
import { ShiftWithAssignments, useDeleteShift, useUnassignVolunteer } from '@workspace/api-client-react';
...
export function ShiftCard({ shift, onEdit, onAssign }: ShiftCardProps) {
    ...
    if (window.confirm('Weet je zeker dat je deze dienst wilt verwijderen?')) {
      deleteShift({ id: shift.id }, {
        onSuccess: () => {
          ...
          toast({ title: "Verwijderd", description: "Dienst is verwijderd." });
        }
      });
    }
  };
  ...
  const handleUnassign = (volunteerId: number, name: string) => {
    if (window.confirm(`Weet je zeker dat je ${name} van deze dienst wilt halen?`)) {
      unassignVolunteer({ id: shift.id, volunteerId }, {
        onSuccess: () => {
          queryClient.invalidateQueries({ queryKey: ['/api/shifts'] });
          toast({ title: "Verwijderd", description: `${name} is van de dienst gehaald.` });
        }
      });
    }
  };
        ...
        <div className="shift-card-header p-4 flex items-center justify-between border-b border-border/40 rounded-t-xl">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              {timeRange && (
                <p className="text-xs font-semibold text-muted-foreground/60 flex items-center gap-1 mt-0.5">
          ...
          {isAdmin && (
            <div className="flex gap-1 no-print">
              <button
                ...
                disabled={isDeleting}
                className="p-2 rounded-lg text-muted-foreground hover:bg-white/70 hover:text-foreground transition-colors"
                title="Bewerken"
              >
  ...
  return (
    <div className={cn(
      "shift-card flex flex-col bg-card rounded-2xl border-2 border-border/60 shadow-sm hover:border-primary/30 hover:shadow-md transition-all duration-300"
    )}>
      {/* Header */}
      <div className="shift-card-header p-4 flex items-center justify-between border-b border-border/60 rounded-t-xl bg-muted/30">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            {timeRange && (
              <p className="text-xs font-semibold text-primary/70 flex items-center gap-1 mt-0.5">
                <Clock className="w-3 h-3" />{timeRange}
              </p>
            )}
            <p className="text-sm font-semibold text-muted-foreground">
              {shift.assignments.length === 0
                ...
                : `${shift.assignments.length} vrijwilliger${shift.assignments.length !== 1 ? 's' : ''} ingedeeld`}
            </p>
          </div>
        </div>
      ...
      <div className="p-4 flex-1 flex flex-col gap-4">
        {shift.notes && (
          <p className="text-sm text-muted-foreground italic bg-muted/50 p-3 rounded-xl border border-border/50">
            "{shift.notes}"
          </p>
        )}
          ...
          {shift.assignments.length === 0 ? (
            ...
            <ul className="space-y-2">
              {shift.assignments.map((assignment) => (
                <li
                  key={assignment.id}
                  className="flex items-center justify-between bg-muted/30 border border-border/50 p-2 pl-3 rounded-lg group print-break-inside-avoid"
                >
                  <span className="font-semibold text-sm">{assignment.volunteer.name}</span>
                  {isAdmin && (
                    <button
                      onClick={() => handleUnassign(assignment.volunteerId, assignment.volunteer.name)}
                      disabled={isUnassigning}
                      className="text-muted-foreground hover:text-destructive p-1 opacity-0 group-hover:opacity-100 transition-opacity no-print"
      ...
      {isAdmin && (
        <div className="p-4 border-t bg-muted/10 rounded-b-xl no-print">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-bold bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground transition-all border border-primary/20 hover:border-primary shadow-sm"
          >
