...
import { useListVolunteers, useAssignVolunteer, type ShiftWithAssignments } from '@workspace/api-client-react';
...
export function AssignModal({ isOpen, onClose, shift }: AssignModalProps) {
  ...
  const unassignedGroupMembers = (selectedVolunteer?.groupMembers ?? []).filter(
    m => !alreadyAssignedIds.has(m.id)
  );
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          {isLoadingVolunteers ? (
            ...
            <div className="p-4 bg-orange-50 border border-orange-200 text-orange-800 rounded-xl">
              Alle vrijwilligers zijn al ingedeeld voor deze dienst.
            </div>
          ) : (
            <select
              required
            ...
            >
                ...
                <optgroup label="Beschikbaar voor dit dagdeel">
                  {availableVolunteers.map(v => (
                    <option key={v.id} value={v.id}>
                      {v.name}{v.groupMembers?.length ? ` 👥 groep (${v.groupMembers.length + 1})` : ''}
                    </option>
                  ))}
                </optgroup>
              )}
              ...
              {unavailableVolunteers.length > 0 && (
                <optgroup label="Niet beschikbaar voor dit dagdeel">
                  {unavailableVolunteers.map(v => (
                    <option key={v.id} value={v.id}>
                      {v.name} (beschikbaar: {v.availability.map(s => getLabel(s)).join(', ') || 'geen'})
                    </option>
                  ))}
                </optgroup>
              )}
        ...
        {selectedVolunteer && unavailableVolunteers.some(v => v.id === selectedVolunteer.id) && (
          <div className="flex items-start gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
            ...
            <span>
              <strong>{selectedVolunteer.name}</strong> is normaal niet beschikbaar voor{' '}
              <strong>{getLabel(shift!.slot)}</strong>. Je kunt hem/haar toch indelen.
            </span>
          </div>
        )}
        ...
        {unassignedGroupMembers.length > 0 && (
          <div className="p-4 bg-primary/5 border-2 border-primary/20 rounded-xl space-y-3">
            <div className="flex items-center gap-2 text-sm font-semibold text-primary">
              ...
              Groep van {selectedVolunteer!.groupMembers!.length + 1} — ook indelen?
            </div>
            <div className="space-y-2">
              {unassignedGroupMembers.map(m => (
                <label key={m.id} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={groupMembersToAssign.has(m.id)}
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-border">
          <button type="button" onClick={handleClose} className="btn-secondary" disabled={isPending}>
            Annuleren
          </button>
          <button
            type="submit"
            className="btn-primary"
            disabled={isPending || !selectedVolunteerId}
          >
